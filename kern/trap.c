...
void trap_init_percpu(void)
{
	...
	struct Taskstate *_ts = &thiscpu->cpu_ts;
	int i = cpunum();

	// Setup a TSS so that we get the right stack
	// when we trap to the kernel.
	_ts->ts_esp0 = KSTACKTOP - i * (KSTKSIZE + KSTKGAP);
	_ts->ts_ss0 = GD_KD;
	_ts->ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + i] = SEG16(STS_T32A, (uint32_t)(_ts),
									sizeof(struct Taskstate) - 1, 0);
	gdt[(GD_TSS0 >> 3) + i].sd_s = 0;

	// Load the TSS selector (like other segment selectors, the
	// bottom three bits are special; we leave them 0)
	// cprintf("trap_init_mp for cpu: %d , load: %x as TSS selector \n", i, GD_TSS0 + (i << 3));
	ltr(GD_TSS0 + (i << 3));
...
void page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall == NULL)
	{
		...
		print_trapframe(tf);
		cprintf("no env_pgfault_upcall set\n");
		env_destroy(curenv);
	}

	uintptr_t saved_esp = tf->tf_esp;
	uintptr_t saved_eip = tf->tf_eip;
	...
	utf->utf_eip = saved_eip;
	utf->utf_err = tf->tf_trapno;
	utf->utf_esp = saved_esp;
	// cprintf("tfesp: %x, saved esp: %x, saved eip: %x \n", tf->tf_esp, saved_esp, saved_eip);
	env_run(curenv);
}
