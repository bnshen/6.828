...
boot_alloc(uint32_t n)
{
	...
	if (n > 0)
	{
		...
		if ((uint32_t)nextfree < 0xf0000000)
		{
			panic("boot_alloc fail");
		}
		return result;
	}
	return NULL;
...
mem_init_mp(void)
{
	...
	{
		cprintf("mem_init_mp for cpu: %d \n", i);
		boot_map_region(kern_pgdir, (uintptr_t)KSTACKTOP - i * (KSTKSIZE + KSTKGAP) - KSTKSIZE,
...
void page_free(struct PageInfo *pp)
{
	...
	{
		panic("pp->pp_ref not zero, is : %d", pp->pp_ref);
	}
...
{
	// Fill this function in
	uintptr_t pdir_index = PDX(va);
	uintptr_t ptable_index = PTX(va);
	pte_t *ptable;
	assert(pdir_index < NPDENTRIES);
	if (pgdir[pdir_index] & PTE_P)
	{
		ptable = KADDR(PTE_ADDR(pgdir[pdir_index]));
	}
	else
	{
		if (create == false)
		...
		{
			struct PageInfo *allocated = page_alloc(ALLOC_ZERO);
			if (allocated)
			{
				allocated->pp_ref += 1;
				ptable = (pte_t *)page2kva(allocated);
				pgdir[pdir_index] = PADDR(ptable) | PTE_P | PTE_U | PTE_W;
			}
			else
			{
				return NULL;
...
void page_remove(pde_t *pgdir, void *va)
{
	...
	pte_t *pte;
	pte_t **pte_store = &pte;
	pgi = page_lookup(pgdir, va, pte_store);
	if (pgi == NULL)
	{
		return;
	}
...
int user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (int i = 0; i <= len; i += PGSIZE)
	{
		...
		if (user_mem_check_addr >= ULIM)
			goto err;
		pte = pgdir_walk(env->env_pgdir, va + i, 0);
		if (pte == NULL)
			goto err;
		if ((*pte & (perm | PTE_P)) == false)
			goto err;
	}
	return 0;
err:
	if ((void *)user_mem_check_addr != va)
