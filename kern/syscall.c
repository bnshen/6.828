...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	// cprintf("set upcall succeed \n");
	assert(e->env_pgfault_upcall);
	return 0;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (pg == NULL)
		return -E_NO_MEM;
	if (page_insert(e->env_pgdir, pg, va, perm))
	{
		page_free(pg);
		return -E_NO_MEM;
	}
			 ...
			 envid_t dstenvid, void *dstva, int perm)
{
	...
	if (envid2env(srcenvid, &src_e, true) | envid2env(dstenvid, &dst_e, true))
		return -E_BAD_ENV;
	if ((uintptr_t)srcva >= UTOP || (uintptr_t)dstva >= UTOP || (uintptr_t)srcva % PGSIZE || (uintptr_t)dstva % PGSIZE)
		return -E_INVAL;
	pte_t *src_pte;
	if (page_lookup(src_e->env_pgdir, srcva, &src_pte) == NULL)
		return -E_INVAL;
	if (PTE_U & perm && PTE_P & perm)
	{
	}
	...
	if (perm & PTE_W && !(*src_pte & PTE_W))
		return -E_INVAL;
	struct PageInfo *pg = pa2page(PTE_ADDR(*src_pte));
	if (page_insert(dst_e->env_pgdir, pg, dstva, perm))
		panic("sys_page_map cannot page_insert()");
	return 0;
